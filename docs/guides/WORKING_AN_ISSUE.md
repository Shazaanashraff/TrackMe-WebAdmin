# Working a GitHub issue — web-admin
...
Issues live on [`Shazaanashraff/TrackMe-WebAdmin`](https://github.com/Shazaanashraff/TrackMe-WebAdmin)
...
```bash
gh issue view <N> --repo Shazaanashraff/TrackMe-WebAdmin
```
...
npm test           # vitest run
npm run lint
npm run test:e2e   # playwright — run when the change touches a page or flow, not just styling
```
...
- That checklist is a **floor, not a ceiling** — if the change touches something it didn't
  anticipate (a new manager/super-admin scoping branch, an API contract), add the test per
  [`ADDING_A_TEST.md`](ADDING_A_TEST.md) and run it too.
- **Manager scoping is not cosmetic** — any change touching a manager-facing page needs a
  regression check that a manager still only ever sees their own resources.
...
- [x] npm test
- [x] npm run lint
- [x] <playwright / anything else the issue or ADDING_A_TEST.md called for>
"
```
...
- [ ] Manager/super-admin scoping unaffected (or explicitly, correctly changed).
