...
export function useEnrollmentRequests(status = 'PENDING', driverId = '') {
  ...
  // The unfiltered pending queue is the freshest answer to "how many are
  // pending", so it feeds the nav badge rather than leaving it on whatever
  // the count call last returned. A driver-filtered queue is a subset, so it
  // must not overwrite the badge with a smaller number.
  const pending = status === 'PENDING' && !driverId && Array.isArray(query.data?.data)
